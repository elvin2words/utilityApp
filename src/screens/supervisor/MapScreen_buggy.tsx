...
import { Alert, Animated as RNAnimated, FlatList, Linking, Modal, Platform, Pressable, ScrollView, StyleSheet, Text, TouchableOpacity, View } from "react-native";
...
import { Plus, Minus, ChevronDown, Compass, Crosshair, Locate } from "lucide-react-native";
...
export default function MapScreen() {
  ...
  const handleNavigateToFault = useCallback(() => {
    if (!selectedFault?.coords) return;
              ...
              faults.map((f) => (
                <>
                  <Marker
                    ...
                    description={`Possible ${f.cause ?? "cause unknown"} | ${f.assignedTo.name ?? "NoOne"} Assigned`}
